...
| 安装 Node.js（可选） | 通过 `INSTALL_NODEJS=false` 跳过，版本通过 `NODEJS_VERSION` 自定义。同时为普通用户配置 npm prefix（`~/.local`）、PATH（`~/.profile`）和 corepack（如可用） |
...
| 分类 | 包名 |
|------|------|
| 基础 | ca-certificates sudo vim nano wget curl git unzip xz-utils gnupg less locales dbus |
...
当 `INSTALL_NODEJS=true` 时，除了通过 NodeSource 全局安装 Node.js，脚本还会为普通用户自动完成以下配置：

| 配置项 | 说明 |
|--------|------|
| npm prefix | 设为 `~/.local`，普通用户 `npm i -g` 无需 sudo |
| PATH | 在 `~/.profile` 中确保 `~/.local/bin` 在 PATH 中（Debian 默认已包含，缺失时自动补写） |
| corepack | 如可用，启用并将 shim（pnpm/yarn）安装到 `~/.local/bin` |
