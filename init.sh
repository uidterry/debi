...
if [ "$INSTALL_NODEJS" = "true" ]; then
    ...
    install -d -o "$USERNAME" -g "$USERNAME" "$USER_HOME/.local/bin"

    su - "$USERNAME" -c 'npm config set --location=user prefix "$HOME/.local"'

    # Debian 默认 .profile 已含 ~/.local/bin PATH，仅在缺失时补写
...
EOF
    ...
    if command -v corepack >/dev/null 2>&1; then
        su - "$USERNAME" -c 'corepack enable --install-directory "$HOME/.local/bin"'
    fi
fi
